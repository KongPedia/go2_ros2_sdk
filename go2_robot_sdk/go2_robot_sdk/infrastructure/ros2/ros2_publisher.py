...
class ROS2Publisher(IRobotDataPublisher):
    ...
    def __init__(
        ...
        self._lidar_queue: "queue.Queue[RobotData]" = queue.Queue(maxsize=1)
    ...
    def publish_lidar_data(self, robot_data: RobotData) -> None:
        ...
        try:
            if self._lidar_queue.full():
                try:
                    _ = self._lidar_queue.get_nowait()
                    self._lidar_queue.task_done()
                except queue.Empty:
                    pass
            self._lidar_queue.put_nowait(robot_data)
        except Exception as e:
            logger.error(f"Error queueing lidar data: {e}")

    def _lidar_worker(self) -> None:
        while True:
            try:
                robot_data = self._lidar_queue.get()
                try:
                    if not robot_data.lidar_data or not self.config.decode_lidar:
                        continue
                    ...
                    publish_rate = float(
                        getattr(self.config, "lidar_publish_rate", 0.0) or 0.0
                    )
                    if publish_rate > 0.0:
                        now = time.monotonic()
                        last = self._lidar_last_publish_ts.get(robot_idx, 0.0)
                        min_period = 1.0 / publish_rate
                        if (now - last) < min_period:
                            continue
                        self._lidar_last_publish_ts[robot_idx] = now
                    ...
                    if points is None:
                        ...
                        if use_cpp and getattr(lidar, "compressed_data", None):
                            try:
                                import lidar_accelerator_v2 as lidar_accelerator
